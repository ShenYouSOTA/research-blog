...
- **Locale URLs (`/zh/…`) ride the same `[slug]` tree** plus the `[slug]/[postSlug]/[...rest]` catch-all — a sibling `[locale]` segment is a Next build error. Dispatcher + params providers live in `src/lib/locale-routes.ts`; content trees under `content/<locale>/` mirror the root (see ARCHITECTURE.md "Locale Content Trees").
...
- **AVIF/WebP images skip the optimizer.** `shouldBypassImageOptimization()` (`src/lib/image-utils.ts`) forces `unoptimized` for `.avif`/`.webp` sources — they render correctly but are served as-is with no responsive srcset (workaround for an upstream `next-image-export-optimizer` AVIF path bug). Use `.jpg` / `.png` when responsive variants matter. Don't add schema-level AVIF rejection — that would contradict the bypass. See `docs/TROUBLESHOOTING.md`.
...
- **Locale trees: the file's location IS its language.** `content/zh/posts/foo.md` twins `content/posts/foo.md` by identical tree-relative path (`treePath`); there is no `lang` frontmatter. Sibling variants (`about.zh.mdx`) THROW with a migration hint. Locale codes (incl. the default) are reserved URL segments — a series/page/alias/basePath named `en`/`zh` fails the build. Generation is sparse: a `/zh/` page exists iff the zh tree holds it. No alias pages under locale prefixes — migration `redirectFrom` sources live at old unprefixed paths, targets localize via entity-carried `getPostUrl(post.locale)`. Never hardcode `'en'`/`'zh'` in locale plumbing (`src/lib/locale-*`, `content/io.ts`); with `defaultLocale: 'zh'` the root tree is Chinese and `/en/` is the prefix.
- **`fix-locale-html-lang.ts` must run BEFORE Pagefind in the build chain.** It stamps `<html lang>` under `out/<locale>/**`; Pagefind splits its index by that attribute (CJK segmentation + no cross-language duplicates). Reordering silently merges the indexes.
...
- **`KatexStyles` must be loaded via `next/dynamic`, never a static import.** A static import executes the `katex.min.css` side-effect at module load, bundling ~23 KB of CSS into every article chunk even for math-free posts. The dynamic boundary in `MarkdownRenderer` / `RstRenderer` is what gives the stylesheet its own chunk.
- **Don't add `'use client'` to a presentational component just for translations.** Use the `T` / `TLocale` / `TLabel` client leaves from `src/components/T.tsx` — they carry the language reactivity (and the one `suppressHydrationWarning` site) while the structural component stays a Server Component. Exception: components that select per-content locale data (like `SimpleLayoutHeader`'s `contentLocales[language]`) are genuinely language-reactive and stay client.
